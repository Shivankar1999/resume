...
const RESUME_DATA = {
  ...
  about:
    "Senior Frontend Developer specializing in high-performance web applications. I turn complex problems into elegant, scalable code. By merging technical architecture with intuitive design, I build interfaces that are as fast as they are functional. My focus is on writing clean, maintainable systems that prioritize the user experience without compromising on performance or accessibility",
  summary:
    "Highly skilled software engineer with 4+ years of experience in React, Next.js. Proven track record of leading teams to deliver scalable web applications that enhance user engagement and drive business growth. Adept at collaborating with cross-functional teams to translate complex requirements into efficient technical solutions.",
  ...
  contact: {
    ...
    social: [
      ...
      {
        name: "LinkedIn",
        url: "https://www.linkedin.com/in/shubham-gaur-565b99215",
        icon: Linkedin,
      },
    ],
  },
  ...
  work: [
    {
      ...
      description:
        "Architected a micro-frontend architecture using Next.js, reducing bundle sizes by 40%. Led a team of 4 engineers.",
    },
    {
      company: "WorldRef.",
      ...
      description:
        "Developed and maintained high-traffic web apps. Optimized database queries resulting in 30% faster load times.",
    },
    {
      ...
      description:
        "Developed a library of 20+ reusable UI components, reducing future development time for new features by ~25% , Optimized image handling and component structure, leading to a 15% improvement in Lighthouse performance scores.",
    },
  ],
  skills: [
    "HTML",
...
const Badge = ({ children, className = "" }) => (
  <span
    className={`inline-flex items-center rounded-md border px-2 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 border-transparent bg-slate-100 text-slate-900 dark:bg-slate-800 dark:text-slate-100 ${className}`}
  >
    {children}
  </span>
);
      ...
      <div className="flex items-center justify-between gap-x-2 text-base">
        <h3 className="inline-flex items-center justify-center gap-x-1 font-semibold leading-none">
          ...
          {badges.map((badge) => (
            <Badge
              key={badge}
              className="align-middle text-[10px] bg-blue-50 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300"
            >
              {badge}
            </Badge>
          ))}
      ...
      <div className="min-h-screen bg-white text-slate-950 dark:bg-slate-950 dark:text-slate-50 transition-colors duration-300">
        <main className="container relative mx-auto scroll-my-12 overflow-auto p-4 md:p-16 print:p-12 max-w-3xl">
                ...
                <div className="flex gap-x-1 pt-1 font-mono text-sm text-slate-500 print:hidden">
                  {RESUME_DATA.contact.social.map((s) => (
                    <a
                      key={s.name}
                      href={s.url}
                      className="p-2 border rounded-md hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
                    >
            ...
            <section className="flex flex-col gap-y-3">
              ...
              <div className="flex flex-wrap gap-1">
                {RESUME_DATA.skills.map((skill) => (
                  <Badge
                    key={skill}
                    className="px-3 py-1 bg-slate-900 text-white dark:bg-white dark:text-slate-900"
                  >
                    {skill}
                  </Badge>
                ))}
            ...
            <section className="flex flex-col gap-y-3 print:break-before-page">
              ...
              <div className="grid grid-cols-1 gap-3 md:grid-cols-2 print:grid-cols-2">
                {RESUME_DATA.projects.map((project) => (
                  <div
                    key={project.title}
                    className="flex flex-col overflow-hidden rounded-lg border border-slate-200 dark:border-slate-800 p-4 space-y-2"
                  >
                    <div className="flex items-center justify-between">
          ...
          <footer className="mt-16 border-t pt-8 text-center text-xs text-slate-400 font-mono print:hidden">
            <p>
              © {new Date().getFullYear()} {RESUME_DATA.name} — Built with
              Next.js & Tailwind
            </p>
          </footer>
        </main>
